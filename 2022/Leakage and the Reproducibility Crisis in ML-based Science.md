...
- モデル情報シートの限界
    1.  計算機による再現性
    2.  モデル情報シートの記載の正誤の判定
    3.  著者と査読者がMLに対する専門知識を持たない分野では、結局Leakageしてるかも
        -   ML適用してみました論文はそうなるな

    4.  新しいLeakageが出たらモデル情報シートも改善していく必要あり
        -   結局、よくあるミスのチェックリストってだけで、問題を解決しきってない



## 主張の有効性の検証方法
- civil war prediction のケースで実際にやってみた
    - 古典的なロジスティック回帰（LR）からランダムフォレストやアダブーストなど複雑なMLモデルに変わってきている
- 本当に性能が改善されているのか、Leakageが修正された後も論文の主張があっているのかを検証してみた
- 結果：
    - トップジャーナルに掲載されている4つの論文すべてでLeakageが見つかり、Leakageを修正したところベースラインのLRモデルからの改善がなかった
    - モデルの性能比較のための郵政検定や不確実性の定量化がされていない（12論文のうち9論文で）



## 批評
- MLの誇大広告におどらされないこと
    - 画像認識では人間超えたとの報告あり→ML使えば自分の分野でもすごい性能がでるんじゃね？

- MLの性能に限界があるということを知ること
    - MLベースの研究の結果は楽観的に捉えないこと
    - MLの専門家を交えて研究すること

- スタンダードな分析のやり方を分野内で確立すること
    - 分野ごとに固有のチェックポイントもあるしね

- 計算の再現性を確認できるようにすること
    - できるだけデータと実験コードをオープンにして第三者のレビューが入るようにすること

- 図1に載っているような、（MLどまんなかの分野ではない）MLを適用・応用してみました論文はあんま信用ならんぞということやな
    - たとえトップジャーナルでも
    - **注意**:図1に載っている論文はおかしい論文に疑問を呈している論文やぞ！
